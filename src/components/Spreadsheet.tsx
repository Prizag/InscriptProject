...
export function Spreadsheet() {
          ...
          <tr className="bg-white sticky top-0 z-20">
            ...
            >
              <div className="flex items-center justify-center space-x-1 text-white">
                ...
                />
                <span className="p-1 text-slate-600">Answer a question</span>
                <EllipsisHorizontalIcon
                  className="w-4 h-4 text-gray-500 cursor-pointer"
  ...
  className={`p-2 border border-gray-300 truncate ${
    ["submitted", "due", "value"].includes(cell.column.id)
      ? "text-right"
      : ["jobRequest", "submitter", "url", "assigned"].includes(cell.column.id)
      ? "text-left"
      : "text-center"
  } 
 `}
  ...
  onClick={(e) => {
    e.stopPropagation();
    console.log(`Cell clicked: Row ${index + 1}, Column "${cell.column.id}"`);
  }}
>
  {flexRender(cell.column.columnDef.cell, cell.getContext())}
          ...
          {emptyRows.map((_, idx) => {
  const globalRowIndex = table.getRowModel().rows.length + idx;
  ...
  return (
    <tr key={`empty-${idx}`}>
      ...
      {columns.map((col, cidx) => {
        const isGlow =
          globalRowIndex === 7 && col.id === "submitted";

        return (
          <td
            key={cidx}
            className={`p-2 border border-gray-300 h-8 ${
  isGlow
    ? "shadow-[0_0_10px_rgba(34,197,94,0.4)] border border-[#6C8B70]"
    : ""
}`}
